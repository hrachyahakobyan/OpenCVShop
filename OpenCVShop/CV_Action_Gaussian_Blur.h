...
namespace core{
	...
	{
	public:
		CV_Action_Gaussian_Blur(const cv::Mat& structuringElement, const cv::Size& ksize,
			double sigma_x, double sigma_y = 0.0, int border_type = 4) :
			structuringElement(structuringElement.clone()), ksize(ksize), sigma_x(sigma_x), sigma_y(sigma_y), border_type(border_type){}
		void operator()(cv::InputArray input, cv::OutputArray output) const;
		std::string description() const;
	public:
		const cv::Mat structuringElement;
		const cv::Size ksize;
